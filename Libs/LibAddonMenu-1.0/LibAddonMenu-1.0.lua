...
function lam:AddHeader(panelID, controlName, text)
	...
	header.data = {
		...
		panel = (isSubMenu and panelID.panel or panelID),
		text = text,
	}
...
function lam:AddSlider(panelID, controlName, text, tooltip, minValue, maxValue, step, getFunc, setFunc, warning, warningText)
	...
	if warning then
		...
		slider.warning.data = { tooltipText = warningText }
	end
...
function lam:AddDropdown(panelID, controlName, text, tooltip, validChoices, getFunc, setFunc, warning, warningText)
	...
	if warning then
		...
		dropdown.warning.data = { tooltipText = warningText }
	end
...
function lam:AddCheckbox(panelID, controlName, text, tooltip, getFunc, setFunc, warning, warningText)
	...
	checkbox.data = {
		...
		system = SETTING_TYPE_UI,
		settingId = _G[strformat("SETTING_%s", controlName)],
		panel = (isSubMenu and panelID.panel or panelID),
		text = text,
		tooltipText = tooltip,
	}
	...
	if warning then
		...
		checkbox.warning.data = { tooltipText = warningText }
	end
...
function lam:AddColorPicker(panelID, controlName, text, tooltip, getFunc, setFunc, warning, warningText)
	...
	colorpicker.data = {
		...
		customSetupFunction = function(colorpicker)
	...
	if warning then
		...
		colorpicker.warning.data = { tooltipText = warningText }
	end
...
function lam:AddEditBox(panelID, controlName, text, tooltip, isMultiLine, getFunc, setFunc, warning, warningText)
	...
	if warning then
		...
		editbox.warning.data = { tooltipText = warningText }
	end
...
function lam:AddButton(panelID, controlName, text, tooltip, onClick, warning, warningText)
	...
	button.data = {
		controlType = OPTIONS_CUSTOM,
		customSetupFunction = function() end, --move handlers into this function? (since I created a function...)
		panel = (isSubMenu and panelID.panel or panelID),
	}
	btn.data = { tooltipText = tooltip }
	...
	if warning then
		...
		button.warning.data = { tooltipText = warningText }
	end
